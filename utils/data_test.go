...
var (
	...
	testPrivA = ed448.NewScalar([]byte{
...
func (s *UtilsSuite) Test_ShakeToScalar(c *C) {
...
func (s *UtilsSuite) Test_AppendBytes(c *C) {
	empty := []byte{}
	bs := []byte{
	...
	c.Assert(AppendBytes(empty, bs, testPrivA, testPubA), DeepEquals, exp)
}

func (s *UtilsSuite) Test_AppendAndHash(c *C) {
	hash := AppendAndHash(testPrivA, testPubA)
...
func (s *UtilsSuite) Test_AppendPoint(c *C) {
	bs := []byte{}
	p := ed448.NewPoint(
		[16]uint32{
...
func (s *UtilsSuite) Test_ExtractPoint(c *C) {
